...
class RenderHelper(object):
    ...
    def global_planner_render(
            self,
            global_plan: list,
            order: str,
    ):
        # Create a table header
        html_table = "<h2>Global Plan</h2>\n"
        html_table += "<table>\n"
        html_table += "<tr><th>Subtask Number</th><th>Subtask Description</th><th>Subtask Action</th><th>Expected State</th></tr>\n"

        # Populate the table with global plan data
        for subtask_info in global_plan:
            html_table += "<tr>"
            html_table += f"<td>{subtask_info['Subtask Number']}</td>"
            html_table += f"<td>{subtask_info['Subtask Description']}</td>"
            html_table += f"<td>{subtask_info['Subtask Action']}</td>"
            html_table += f"<td>{subtask_info['Expected State']}</td>"
            html_table += "</tr>\n"

        html_table += "</table>\n"

        # Open the existing HTML file and update its content
        self.render_file.seek(0)
        html = self.render_file.read()
        html_body = re.findall(r"<body>(.*?)</body>", html, re.DOTALL)[0]

        # Insert the global plan table into the HTML content
        html_body += html_table

        # Update the HTML content and save it to the file
